// Character data: a juicy lineup of noble families across Westeros
const characters = [
...
// mapNameFamily: introduce each character like royalty
const mapNameFamily = (characters) => {
  // Transform each object into a sentence: "Arya, of House Stark"
  return characters.map((char) => `${char.name}, of House ${char.house}`);
};

// filterFamily: extract only characters from a target house
const filterFamily = (characters, house) => {
  // Filter characters whose house matches the parameter
  return characters.filter((char) => char.house === house);
};

// reduceHouses: count how many members are repping each house
...
// [
//   'Arya, of House Stark', 'Sansa, of House Stark', ...
// ]
